...
namespace Арканоид
{
    public partial class Form1 : Form
    {
        ...
        {
            Bitmap pictures = Properties.Resources.arcanoidres;
            for (int i = 0; i < mapHeight; i++) 
            {
                for (int j = 0; j < mapWidth; j++)
                {
                    if (map[i, j] == 9)
                    {
                        ...
                        g.DrawImage(pictures, new Rectangle(new Point(j * 20, i * 20), new Size(60, 20)), 398, 17, 150, 50, GraphicsUnit.Pixel);
                    }

                    if (map[i, j] == 8)
                    {
                        //отрисовка мячика
                        g.DrawImage(pictures, new Rectangle(new Point(j * 20, i * 20), new Size(20, 20)), 806, 548, 73, 73, GraphicsUnit.Pixel);
                    }

                    //отрисовка панелек
                    if (map[i, j] == 1)
                    {
                        g.DrawImage(pictures, new Rectangle(new Point(j * 20, i * 20), new Size(40, 20)), 20, 16, 170, 59, GraphicsUnit.Pixel);
                    }

                    if (map[i, j] == 2)
                    {
                        g.DrawImage(pictures, new Rectangle(new Point(j * 20, i * 20), new Size(40, 20)), 20, 16 + 77 * (map[i, j] - 1), 170, 59, GraphicsUnit.Pixel);
                    }
